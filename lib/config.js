...
export default function config() {
  ...
  custom.exts = _.chain(exts).difference(DEFAULT_EXTS).uniq().value();

  let exclusions = atom.config.get('livereload.exclusions').split(',').map( ex => ex.trim() );
  exclusions = _.chain(exclusions).difference(...DEFAULT_EXCLUSIONS).uniq();
  exclusions = exclusions.map( pattern => { return new RegExp(pattern.replace(/([.\\\/])/g, '\\$1' )) } ).value();
  custom.exclusions = [...exclusions, /\.DS_Store/, /\.gitignore/, /\.git\//, /\.svn\//, /\.hg\//];
